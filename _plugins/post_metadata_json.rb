...
module Jekyll
  ...
  class PostMetadataJsonGenerator < Generator
    ...
    def self.schema_for(site, post)
      ...
      description = strip_html(post.data["description"] || post.data["excerpt"] || post.excerpt || site.config["description"])
      author_name = author_name(site, post.data["author"])
      language = site.config["locale"].to_s[0, 2]
      image = post.data["image"] || post.data.dig("header", "image") || site.config["teaser"] || site.config["og_image"]
      ...
      schema = {
        ...
        "publisher" => {
          "@type" => site.config.dig("social", "type") || "Organization",
          "name" => site.config.dig("social", "name") || site.config["title"],
          "url" => site_url
        },
        "isPartOf" => {
          "@type" => "Blog",
          "@id" => "#{site_url}/archives/",
          "name" => site.config["title"],
          "url" => "#{site_url}/archives/"
        }
      }
      schema["image"] = absolute_url(site_url, image) if image
